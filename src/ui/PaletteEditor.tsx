...
function PaletteRow({
  color, index, total, onChangeColor, onRemove, onMoveUp, onMoveDown, disabled,
}: {
  ...
  useEffect(() => { setDraft(color); setHexError(false); }, [color]);
  ...
  return (
    <div className="palette-row">
      ...
      <div className="palette-row__text">
        <input
          type="text" value={draft} aria-invalid={hexError}
      ...
      <button className="btn-icon" type="button" onClick={onRemove} disabled={disabled} aria-label="Remove">×</button>
    </div>
  );
}

export function PaletteEditor({
  colors, bgColor, onChangeBackground, onChangeColor,
  onAddColor, onRemoveColor, onMoveColor, onLoadPalette, onRandomizePalette,
}: PaletteEditorProps) {
  return (
    <>
      ...
      <div className="flex flex-col gap-2" style={{ marginBottom: 12 }}>
        {colors.map((color, index) => (
          <PaletteRow
            key={`${color}-${index}`}
            color={color} index={index} total={colors.length}
            onChangeColor={(v) => onChangeColor(index, v)}
        ...
        <div className="flex items-center gap-2" style={{ marginTop: 4 }}>
          <span className="field-label" style={{ margin: 0, flex: 1 }}>Background</span>
          <input type="color" value={bgColor} onChange={(e) => onChangeBackground(e.target.value)} />
        </div>
      </div>
      ...
      <div className="flex gap-2">
        {PRESET_PALETTES.map((palette, i) => (
          <button key={i} className="preset-swatch" type="button"
            onClick={() => onLoadPalette(palette)} aria-label={`Preset ${i + 1}`}>
